...
const StrokeAssessment = ({
    ...
    const loadModel = async () => {
      try {
        ...
        setDebugInfo((prev: any) => ({
          ...prev,
          modelStatus: "Loading TensorFlow...",
        }));
        ...
        setDebugInfo((prev: any) => ({
          ...prev,
          modelStatus: "Loading FaceMesh model...",
        }));
  ...
  const startCamera = async () => {
    try {
      ...
      setDebugInfo((prev: any) => ({
        ...prev,
        cameraStatus: "Requesting access...",
      }));
  ...
  const analyzeFacialSymmetry = useCallback(async () => {
    if (!videoRef.current || !canvasRef.current) {
      setDebugInfo((prev: any) => ({
        ...prev,
        error: "Video or canvas not ready",
      }));
      return null;
    }
      ...
      if (result.faces_detected > 0) {
        ...
        if (canvasRef.current && result.face_box) {
          ...
          if (ctx) {
            ...
            ctx.fillStyle = result.droop_detected
              ? "rgba(255, 0, 0, 0.8)"
              : "rgba(0, 255, 0, 0.8)";
  ...
  const analyzeSpeechClarity = (transcript: string) => {
    ...
    const hasSlurring =
      normalizedTranscript.length < expectedPhrase.length * 0.7;
    const wordsMatch =
      normalizedTranscript.includes("sky") &&
  ...
  const completeAssessment = useCallback(async () => {
    ...
    const strokeDetected =
      strokeIndicators.facialDroop ||
    ...
    if (strokeDetected) {
      ...
      await speak(
        "No stroke detected. You will now proceed to symptom reporting."
      );
    }
    ...
    console.log(
      "🚀 Calling onStrokeDetected callback with data:",
      assessmentData
    );
    onStrokeDetected(assessmentData);
  }, [assessmentResults, onStrokeDetected, speak, user._id]);
        ...
        const faceAnalysis =
          detections.length > 0
            ? detections.reduce((best, current) =>
                parseFloat(current.asymmetryScore) >
                parseFloat(best.asymmetryScore)
                  ? current
                  : best
              )
            : null;
  ...
  }, [currentStep, speak, startListening, analyzeFacialSymmetry, completeAssessment]);
  ...
  useEffect(() => {
    if (model && cameraReady && currentStep < FAST_QUESTIONS.length) {
      console.log(
        `🎬 Starting question ${currentStep + 1}/${FAST_QUESTIONS.length}`
      );
      processQuestion();
    } else if (!model) {
        ...
        <div className="assessment-content">
          ...
          {showDebug && (
            ...
            >
              ...
              <div>
                Video Ready: {debugInfo.videoReady ? "✅ YES" : "❌ NO"}
              </div>
              ...
              {debugInfo.videoState && (
                <>
                  ...
                  >
                    Video: {debugInfo.videoState.videoWidth}x
                    {debugInfo.videoState.videoHeight}
                  </div>
              ...
              <div>
                Current Step: {currentStep + 1}/{FAST_QUESTIONS.length}
              </div>
              {debugInfo.faceAnalysis && (
                <>
                  ...
                  <div>
                    Asymmetry Score: {debugInfo.faceAnalysis.asymmetryScore}
                  </div>
                  <div>
                    Mouth Asymmetry: {debugInfo.faceAnalysis.mouthAsymmetry}
                  </div>
                  <div>
                    Eye Asymmetry: {debugInfo.faceAnalysis.eyeAsymmetry}
                  </div>
                  <div>
                    Droop Asymmetry: {debugInfo.faceAnalysis.droopAsymmetry}
                  </div>
                  <div
                    style={{
                      color: debugInfo.faceAnalysis.droopDetected
                        ? "#ff0000"
                        : "#00ff00",
                    }}
                  >
                    Droop Detected:{" "}
                    {debugInfo.faceAnalysis.droopDetected ? "⚠️ YES" : "✅ NO"}
                  </div>
